...
AMALTHEA-based models to analyze timming behaviors of CUDA kernels on
...
Our algorithm is based on these rules and our experimental result shows
the accuracy of our algorithm to estimate completion times for kernels
executed on Jetson TX2 platform. Real timming data from NVIDIA's
platform was used in our experiments. Moreover, we implemented our
algorithm on Eclipse APP4MC, which allows AMALTHEA-based response time
...
apply to our study case. We discuss abour Jetson TX2's memory hierarchy
...
In this chapter, we present our approach to calculate the response time
...
assumptions, and give an brief introduction to GPU response time
analysis.\\
In addition we introduce our algorithm, show some examples, describe how
algorithm can change under certain conditions, and discuss its
...
a period \(T_i\) defined as the separatin between two consecutivs
...
assume that all the blocks, regarless the kernel, are the same size.
...
In Figure \ref{img:ta_example} we present two cases. Let's assume there
...
kernel K3 with 6 blocks \(g_3 = 6\) is going to be allocation on the
...
following kernel. Let's start with \(g_f\), it is easy to notice that
...
The next logic question is where the last block of K3 should be
...
each block regarless from which kernel \(t_i\) comes. In addition, it is
...
Fortunately, it is necessary only to track \(g_f\) at specific points of
...
updated values of \(g_f\) and \(t_a\) depend as well on these them.

Let's define a set \(h\) of pair of values \((t_k, g_k)\) where \(g_k\)
...
Let's say there are four kernels we want to allocated, all with the same
...
\(g_f = g_{max} = 8\). Let's start with \(\tau_1\). Kernel \(\tau_1\)
...
not known yet. Let's continue with the analysis.
...
In Figure \ref{img:ex_2}(c) is shown the GPU state, \(t_a\) and \(g_f\)
...
Since we already described step by step analysis for \(\tau_1\) and
...
that all kernel were scheduled at the same time, the release time for
...
other case. If Figure \ref{img:free_blocks_alg} is shown the block
distribution when \(g_i > g_f\), thus in order to find \(g^{*}_f\) and
...
The value of \(t^{*}_a\) is calculated in similar fashion and is given
...
We analyze in more details the influence of \(g_{max}\) and maximum
number of allocated blocks on computation time. Let us assume that
number of blocks per kernel increase in linear fashion to the number of
kernels. In order words, the more kernels we want to launch, the more
...
Bosch WATERS Challenge 2019, explained the architecture of NVIDIA Jetson
